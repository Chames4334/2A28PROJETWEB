    ...
    <div class="page-shell">
        ...
        <section class="form-card">
            ...
            <form id="form-conge" method="post" action="?action=edit&id=<?php echo $conge['id_conge']; ?>" onsubmit="return validateCongeForm();">
                ...
                <input type="date" id="date_debut" name="date_debut" value="<?php echo htmlspecialchars($_POST['date_debut'] ?? $conge['date_debut']); ?>">

                <label for="date_fin">Date de fin (AAAA-MM-JJ)</label>
                <input type="date" id="date_fin" name="date_fin" value="<?php echo htmlspecialchars($_POST['date_fin'] ?? $conge['date_fin']); ?>">

                <label for="type_conge">Type de congé</label>
                <select id="type_conge" name="type_conge">
                    <option value="Congé payé" <?php echo (($conge['type_conge'] ?? '') == 'Congé payé') ? 'selected' : ''; ?>>Congé payé</option>
                    <option value="Congé maladie" <?php echo (($conge['type_conge'] ?? '') == 'Congé maladie') ? 'selected' : ''; ?>>Congé maladie</option>
                    <option value="Congé sans solde" <?php echo (($conge['type_conge'] ?? '') == 'Congé sans solde') ? 'selected' : ''; ?>>Congé sans solde</option>
                </select>
